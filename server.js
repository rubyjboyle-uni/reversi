...
io.on('connection', (socket) => {
    ...
    //count: the number of users in chat room
...
//uninvite
    ...
    socket.on('game_start', (payload) => {
        ...
        //make sure engaged player present
        io.in(room).allSockets().then((sockets) => {
            //engaged not in room
            if ((typeof sockets == 'undefined') || (sockets === null) || !sockets.has(requested_user)) {
                response = {
                ...
                socket.emit('game_start_response', response);
                serverLog('game_start command failed', JSON.stringify(response));
            } else { //engaged player is in the room
    ...
    socket.on('play_token', (payload) => {
        ...
        //make sure the current play is from the right player
        if (
...
//return true if r + dr supports playing at r and c +dc 
function adjacent_support(who, dr, dc, r, c, board) {
    ...
    if (who === 'b') {
        ...
        log("Houston we have a prob: " + who);
        return false;
    }

    //check to make sure that the adacent support is on board
    if(( r + dr < 0 ) || ( r + dr > 7 )) {
        return false;s
    }
    ...
    //check to make sure there is space for matching color
